title: "PROJECT ON MEDICAL DEVICES CASE STUDY USING R CODE"
...
setwd("D:/kanchan/19 june R Batch/PROJECT")
...
str(dataset) #part of base function

# glimpse gives you a better organized structure of the data.

glimpse(dataset)  #Function coms in DPLYR



# Using the describe() function, we can compute descriptive 
# statistics for numerical data. 
# The descriptive statistics help determine the distribution of 
# numerical variables.

summary(dataset)

describe(dataset) #this comes from psych
...
# **262** missing values are really high in number. Therefore, 
# it is essential to delete these columns as they dont contribute 
...
# Lets use the **plot** function to do **univariate analysis** 
# of some variables.
...
# Majority of patients have normal levels of **glycated hemoglobin**. 
# However, higher levels of **glyhb** seems to be evenly spread across 
# the sample size.

plot(dataset$stab.glu)

# Same goes with the **stabilized glucose** levels. 
# Ultimately, these patterns should match because the end goal is same 
# although the scales are different.

plot(dataset$age)

# We get from this chart patients from different **age groups** and the sample distribution seems to be 
# evenly spread.
...
# **Female** patients are more as compared to the **Male** patients. 
# Here, we can conclude that the prevalence of diabetes is more 
# in women in comparison to men.

# But, wait a minute, dont jump to conclusions in such an early stage 
# of analysis. What I'll suggest is to lookout for some metrics or 
# statistics before you draw some conclusion.

barplot(table(dataset$frame))
# or we can use ggplot for the same
ggplot(dataset, aes(x=frame)) + geom_bar()

# **Frame** variable basically talks about the body frames of the 
# patients. Here, maximum number of pateints belong to **meduim** 
#   body frame.
...
# These are the two **locations** where Glumet is planning to 
# launch the product. Apparemtly, Louisa seems to have more number of 
# target customers.
...
# Although, these two variables seem highly correlated but 
# however, it is important to understand the scales of glucose 
# levels we are looking at.

# To have a better picture, lets see who our **target customers** 
# are in colorful visualization.
...
# Lets see how the data is distributed using the **box plots**, 
...
# 
# Although, the spread of our potential customers is across different 
# age groups but, primarily we need to focus on **Diabetic** and 
# **Pre-Diabetes** patients.


# Here we can clearly see our target customers with respect to the 
# age groups we are looking at.
...
# Again, **group 2** has got all our potential customers.
...
# We can see some trend for **glyhb** to increase with **age**, and 
# we tend to 
# see **medium** and **large** body frame people at higher levels of glyhb.
...
# It is quite evident that age **Group 2** customers have elevated levels 
# of **glyhb** with **large body frame** and **heavy weight**.


#Conclusion
# What we observe here is that people with more body **weight** tend to
# be **diabetic**. Hence, we can take an **operational decision** here 
...
# It is obvious that **heavy weight** is **directly proportional** to the 
# **large body frame**.
...
# From the above bar plot, we can say that females are more prone 
# to diabetes in comparision to males.

ggplot(dataset, aes(x=gender, fill=frame)) + geom_bar()

# However, It seems there are more number of **males** with 
# **large** body **frame**

# We know that there are more number of female patients than male. 
# But, lets just see those numbers in percentages.
...
# **59%** is a good number which tells us on which gender we should 
# keep our focus.
# 
# Based on gender, lets see what are the percentages in 
# different **locations**.
...
# I think we have very good infromation here, our primary focus in 
# gender **male** should primarily be in **Buckingham** 
#   because the glyhb levels are really high there.  
...
# **Buckingham** is the place where we should **focus** more on the 
# **large** body frame as it holds **60%** share.
# Moreover, we have **56%** of **medium** body frame people to look in 
# **Louisa*


# Now, lets see what are the percentages of body frames in 
# different locations. 

vtree(dataset,"frame location",horiz=FALSE,height=250,width=850,showlevels=FALSE)

# For **large** and **medium** body frames **Buckingham** should be our 
# priority as per the **mean** glyhb levels, whereas, 
# for **small** body frame we can focus more on **Louisa**

# 7. Strategy and Operations

# In order to come up with **strategy** which will help us take operational 
# decisions, what we are doing here is to transform the **result** 
# variable which is technically a binary variable with ones and zeros. 
# We will convert this into a factor variable with values **Positive** 
# which is diabetic and **negative** means non-diabetic. 
...
# Before that, lets see our **diagnosis** variable which tells us about 
# the percentage of **Diabetic**, **Non-Diabetic**, and **Pre-Diabetes** 
# people. 


vtree(dataset, "diagnosis", horiz=FALSE, height=250, width=850)

# As you can see, the percentage of diabetic customers (**18%**) are 
# more as compared to the pre-diabetes (**10%**) which is a good sign for 
# our launch processs.


# Now, lets see the percentage of target customers we have based on the 
# **result**
vtree(dataset, "result", horiz=FALSE, height=250, width=850)

# If we combine the diabetic and pre-diabetic percentages then its going to 
# give us **28% Positive** results which is a very good number to launch 
# our product.

# To prioritize our operations, lets look at the variable result with 
# respect to **location**.
vtree(dataset, "result location", height=250, width=850)

#Buckingham is the place to start the launch
